...
var dragger = function () {
  ...
  move = function (dx, dy) {
    try {
      ...
      this.parent.raphaelTaskText.transform('t2,' + this.parent.text_marging);
...
function maestro_draw_task(taskInformation) {
  ...
  var task_text = get_raphael_text(taskInformation.uilabel ? taskInformation.uilabel:'', task_width);

  task.raphael = r.rect(Number(taskInformation.left), Number(taskInformation.top), task_width, task_text.height > 60 ? task_text.height:60, 2);
  task.raphael.parent = taskInformation;
  task.raphael.parent.text_marging = 22 + (task_text.lines > 1 ? 24 : 0);
  task.raphael.parent.pointedfrom = [];
  //create a rect around the top
  task.raphael.parent.toprectraphael = r.rect(Number(taskInformation.left), Number(taskInformation.top), task_width, 15, 2).attr({fill: '#a0a0a0',  'fill-opacity': 90});
  sampleText = taskInformation.taskname;
  ...
  tasktext = r.text(Number(taskInformation.left) + 4, Number(taskInformation.top) + task.raphael.parent.text_marging , task_text.text).attr({'font-weight': 'bold', fill: '#000000', 'text-anchor': 'start'});
...
(function($, Drupal) {
  Drupal.AjaxCommands.prototype.addNewTask = function(ajax, response, status) {
    ...
    $('[name="task_clicked"]').val(response.id);

    $('[name="task_top"]').val(15);
    $('[name="task_left"]').val(15);
    $('#edit-add-task-complete').trigger('mousedown'); //trigger the ajax event wired to this button
  }
  ...
  Drupal.AjaxCommands.prototype.maestroUpdateMetaData = function(ajax, response, status) {
    ...
    $('[name="task_clicked"]').val(response.taskid);
    $('#edit-update-task-complete').trigger('mousedown'); //trigger the ajax event wired to this button
...
function get_raphael_text(txt, max_width) {
  ...
  return {
    text: lines.join("\n"),
    height: row_height * (lines.length + 2),
    lines: lines.length
  };
}
